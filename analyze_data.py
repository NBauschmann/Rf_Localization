...
heat_map_tag_id = 6  # heat map fuer welches tag ?
...
def analyze_data(b_onboard=False, measfilename='path'):
    ...
    offset_camera = [160, 90, 300]  # [160, 105, 300]
    ...
    if Old_or_new == 'new':
        ...
        # 45 Degree Tag between w3 and w4:
        rotation_45_deg = Quaternion(axis=[0, 1.0, 0], degrees=-45)
        tag_w3_w4_orientation =  tag_w3_orientation * rotation_45_deg
        ...
        # neu ueber wasser
        tag_3 = tc.Tag(3, np.array([-120, 60, 139]), tag_w3_w4_orientation)
    ...
    wp_error_abs = []   # for heatmap
    heat_map_matrix = np.ndarray(shape=(stepx, stepy))
        ...
        for tag in range(num_seen_tags):
            ...
            if meas_type == 0:  # 2d plot und messung
                ...
                if meas_type == 2:  # 3d measurement, unknown camera orientation
                    ...
                    # Umrechnung mit Methoden der Tag Klasse:
                    ...
                    # Umrechnung mit (vorgegebener Drehmatrix) / vorgegeben Quaternion
                    #S_tc = np.array([[1.0, 0, 0], [0, -0.985, 0.174], [0, -0.174, -0.985]])
                    #q_tc = Quaternion(-0.0436194, 0.9990482, 0, 0 )  # 185 Grad um x-achse gedreht
                    #q_tc = Quaternion(0, 1.0, 0, 0)  # fuer Messung ohne Brotdose: Kamera schaut (relativ) gerade nach vorne
                    #position_wf = tags[tag_id].convert_location_to_wf(q_tc.normalised, dist_cam_tag_cf)

                    # Umrechnung mit aus Quaternion berechneter Drehmatrix
                ...
                #print all_meas_data[line][0][0]
                #x_wp = float(all_meas_data[line][0][0])
                wp = np.array([all_meas_data[line][0][0], all_meas_data[line][0][1], all_meas_data[line][0][2]])   # including offset camera
                wp_meas = np.array([position_wf[0], position_wf[1], position_wf[2]])
                wp_meas_diff = np.subtract(wp, wp_meas)
                #print wp_meas_diff
                wp_error_abs = np.linalg.norm(wp_meas_diff)
                #print wp_error_abs

                x = all_meas_data[line][1][1]
                if all_meas_data[line][1][1] == 3:   # erstmal test nur fuer die jeweils erste messung
                    if tag_id == heat_map_tag_id:   # erstmal nur fuer ein tag
                        num_wp = all_meas_data[line][1][0]
                        num_wp_x = num_wp % stepx
                        num_wp_y = int(num_wp / stepx)
                        #print num_wp_x, num_wp_y
                        heat_map_matrix[num_wp_x, num_wp_y] = wp_error_abs
    ...
    if error_plot:    # erstmal nur fuer Messung mit 4 Tags

        average_error_x_3 = math.sqrt(sum(x_error_tag_3) / float(len(x_error_tag_3)))
        average_error_y_3 = math.sqrt(sum(y_error_tag_3) / float(len(y_error_tag_3)))
        average_error_z_3 = math.sqrt(sum(z_error_tag_3) / float(len(z_error_tag_3)))
        np.std(x_error_tag_3)

        ind = np.arange(3)
        fehler = (average_error_x_3, average_error_y_3, average_error_z_3)
        p1 = plt.bar(ind, fehler)
        #plt.xticks(ind, )
        plt.ylabel('Fehler in [mm]')
        #plt.legend((), (''))
        plt.show()
        plt.bar()
    ...
    if heat_map_plot:
        fig2 = plt.figure()
        ax = sns.heatmap(heat_map_matrix, cmap="Greens")
        plt.show()
