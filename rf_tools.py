...
def analyze_measdata_from_file(b_onboard=False, measfilename='path'):#model_type='log', analyze_tx=[1, 2, 3, 4, 5, 6],  meantype='db_mean', b_onboard=False, measfilename='path'):
    ...
    # print wp_mat[:, 0]
    ax.plot(wp_mat[:, 0], wp_mat[:, 1], wp_mat[:, 2], '.', color='#9f9696')
    """
    if meas_type == 0:
    ...
    for t in range(len(tags)):
        ax.scatter(tags[t].get_position_wf()[0], tags[t].get_position_wf()[1], tags[t].get_position_wf()[2], color='#660066')   # MuM purpur
        ...
        for tag in range(num_seen_tags):
            ...
            if meas_type == 0:   # 2d plot und messung
                ...
                if meas_type == 1:    # 3d measurement, known camera position
                    dist_cam_tag_wf = cam_orientation.rotate(np.array(all_meas_data[line][2][tag][1:4]))  # vector cam-tag in world frame
                ...
                if meas_type == 2:     # 3d measurement, unknown camera orientation
                    ...
                    position_wf = tags[tag_id].convert_measurement_to_wf(orientation_cam_tag, dist_cam_tag_cf)  # vector cam-tag in world frame
    ...
    ax.plot(xs_1, ys_1, zs_1, '.', color='#008c00', label='Tag 1')#, facecolor='#008c00')  # MuM Gruen
    ...
    tag1 = Line2D(range(1), range(1), color="white", marker='o', markerfacecolor="#66ff66",
                        markeredgecolor="#009900")
    tag2 = Line2D(range(1), range(1), color="white", marker='o', markerfacecolor="#6666ff",
                       markeredgecolor="#0000cc")
    tag3 = Line2D(range(1), range(1), color="white", marker='o', markerfacecolor="#ffb266",
                         markeredgecolor="#994c00")

    plt.legend((Tags, wp, tag1, tag2, tag3), ('Tags', 'x', 'Partikel1', 'Partikel2'),
                   numpoints=1, loc=1)
...
"""
analyze_measdata using new tag class and apriltags2_ros data

"""

def analyze_measdata_from_file_new(b_onboard=False,
                               measfilename='path'):
    """
        set meas_type: 
        meas_type = 0: 2D measurement (plot)
    ...
    # print wp_mat[:, 0]
    ax.plot(wp_mat[:, 0], wp_mat[:, 1], wp_mat[:, 2], '.', color='#9f9696')  # MuM Grau
    """
    if meas_type == 0:
        for t in range(len(tags)):
            ax.scatter(tags[t][0], tags[t][1], tags[t][2], edgecolor='#cc0000', facecolor='#cc0000')
    ...
    for t in range(len(tags)):
        ax.scatter(tags[t].get_position_wf()[0], tags[t].get_position_wf()[1], tags[t].get_position_wf()[2],
                   color='#660066')  # MuM purpur
        ...
        for tag in range(num_seen_tags):
            ...
            if meas_type == 0:  # 2d plot und messung
                ...
                if meas_type == 2:  # 3d measurement, unknown camera orientation
                    ...
                    position_wf = tags[tag_id].convert_location_to_wf(orientation_cam_tag, dist_cam_tag_cf)  # vector cam-tag in world frame in mm
                    #print position_wf
    ...
    print (
    'average error tag 1 in x, y, z: ' + str(math.sqrt(sum(x_error_tag_1) / float(len(x_error_tag_1)))) + ', ' + str(
        math.sqrt(sum(y_error_tag_1) / float(len(y_error_tag_1)))) + ', ' + str(
        math.sqrt(sum(z_error_tag_1) / float(len(z_error_tag_1)))))
    """print (
    'average error tag 2 in x, y, z: ' + str(math.sqrt(sum(x_error_tag_2) / float(len(x_error_tag_2)))) + ', ' + str(
        math.sqrt(sum(y_error_tag_2) / float(len(y_error_tag_2)))) + ', ' + str(
        math.sqrt(sum(z_error_tag_2) / float(len(z_error_tag_2)))))
    print (
    'average error tag 3 in x, y, z: ' + str(math.sqrt(sum(x_error_tag_3) / float(len(x_error_tag_3)))) + ', ' + str(
    ...
    plt.legend((Tags, wp, tag1, tag2, tag3), ('Tag', 'Wegpunkt', 'Messung Tag 0', 'Messung Tag 1'),
               numpoints=1, loc=1)
